...
- **CSP-Root-Cause gefunden (2026-06-04):** Die Live-CSP setzte `connect-src 'self'` → der Cross-Origin-`fetch` des Formulars an das Backend (`…run.app`) wurde **vom Browser geblockt** (curl-Preflight 200 zählt nicht — CSP ist browser-only). Der Antigravity-Commit `7edaf77` änderte nur die Komponente, nicht die CSP. **Fix:** `connect-src` um die API-Origin erweitert in `nginx.conf` (4×) **und** in der neuen `firebase.json`. → vor echtem Test deployen.
