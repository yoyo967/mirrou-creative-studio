...
<!-- Self-hosted assets (CSP-safe, offline-capable; were Google Fonts + jsDelivr + unpkg CDNs) -->
...
<div id="main">
  <div class="toolbar">
    ...
    <div class="tr">
      ...
      <div class="divider-v"></div>
      <button class="tb" onclick="openCommandPalette()" title="Schnellsuche · Strg/Cmd + K"><i data-lucide="command"></i><span class="tb-label">Suche (⌘K)</span></button>
...
/* ═══ ⌘K Command Palette (client-side fuzzy doc search; CSP-safe) ═══ */
(function(){
  var style=document.createElement('style');
  style.textContent='#cmdk-overlay{position:fixed;inset:0;z-index:300;background:rgba(3,3,3,.62);backdrop-filter:blur(5px);display:none;align-items:flex-start;justify-content:center;padding-top:13vh;}'
    +'#cmdk-overlay.open{display:flex;}'
    +'#cmdk-box{width:min(660px,92vw);background:var(--surface,#0e0e10);border:1px solid var(--border,#1C1C22);border-radius:14px;box-shadow:0 24px 80px rgba(0,0,0,.6);overflow:hidden;}'
    +'#cmdk-input{width:100%;box-sizing:border-box;background:transparent;border:none;border-bottom:1px solid var(--border,#1C1C22);padding:18px 22px;color:var(--text,#F2EFE9);font-size:16px;outline:none;font-family:var(--font-sans,Inter,sans-serif);}'
    +'#cmdk-results{max-height:54vh;overflow-y:auto;padding:8px;}'
    +'.cmdk-item{display:flex;align-items:center;gap:12px;padding:11px 15px;border-radius:9px;cursor:pointer;}'
    +'.cmdk-item.sel{background:rgba(168,85,247,.15);}'
    +'.cmdk-item .ck-t{font-size:14px;color:var(--text,#F2EFE9);font-weight:600;font-family:var(--font-sans,Inter,sans-serif);}'
    +'.cmdk-item .ck-c{font-size:10px;text-transform:uppercase;letter-spacing:1px;color:var(--text-3,#6B7280);margin-left:auto;font-family:var(--font-mono,monospace);white-space:nowrap;}'
    +'.cmdk-empty{padding:30px;text-align:center;color:var(--text-3,#6B7280);font-size:13px;}'
    +'#cmdk-hint{padding:9px 18px;border-top:1px solid var(--border,#1C1C22);font-size:10px;color:var(--text-3,#6B7280);font-family:var(--font-mono,monospace);display:flex;gap:18px;}';
  document.head.appendChild(style);
  var ov=document.createElement('div');ov.id='cmdk-overlay';
  ov.innerHTML='<div id="cmdk-box"><input id="cmdk-input" type="text" placeholder="Dokument suchen …" autocomplete="off" spellcheck="false"><div id="cmdk-results"></div><div id="cmdk-hint"><span>↑↓ Navigieren</span><span>↵ Öffnen</span><span>Esc Schließen</span></div></div>';
  document.body.appendChild(ov);
  var input=ov.querySelector('#cmdk-input'),results=ov.querySelector('#cmdk-results'),matches=[],sel=0;
  function esc(t){return (t||'').replace(/[&<>]/g,function(c){return c==='&'?'&amp;':c==='<'?'&lt;':'&gt;';});}
  function build(q){
    q=(q||'').trim().toLowerCase();
    matches=DOC_KEYS.filter(function(k){var d=DB[k];if(!d)return false;
      return !q||d.title.toLowerCase().indexOf(q)>=0||(d.desc||'').toLowerCase().indexOf(q)>=0||(d.content||'').toLowerCase().indexOf(q)>=0;});
    if(q)matches.sort(function(a,b){var ia=DB[a].title.toLowerCase().indexOf(q),ib=DB[b].title.toLowerCase().indexOf(q);
      return (ia<0?999:ia)-(ib<0?999:ib);});
    matches=matches.slice(0,40);sel=0;renderList();
  }
  function renderList(){
    if(!matches.length){results.innerHTML='<div class="cmdk-empty">Keine Treffer</div>';return;}
    var h='';for(var i=0;i<matches.length;i++){var d=DB[matches[i]];
      h+='<div class="cmdk-item'+(i===sel?' sel':'')+'" data-i="'+i+'"><span class="ck-t">'+esc(d.title)+'</span><span class="ck-c">'+esc(d.cat||'')+'</span></div>';}
    results.innerHTML=h;var s=results.querySelector('.cmdk-item.sel');if(s)s.scrollIntoView({block:'nearest'});
  }
  function openPal(){ov.classList.add('open');input.value='';build('');setTimeout(function(){input.focus();},30);}
  function closePal(){ov.classList.remove('open');}
  function choose(i){if(matches[i]){closePal();openDoc(matches[i]);}}
  input.addEventListener('input',function(){build(input.value);});
  results.addEventListener('click',function(e){var it=e.target.closest('.cmdk-item');if(it)choose(parseInt(it.dataset.i,10));});
  ov.addEventListener('click',function(e){if(e.target===ov)closePal();});
  input.addEventListener('keydown',function(e){
    if(e.key==='ArrowDown'){e.preventDefault();sel=Math.min(sel+1,matches.length-1);renderList();}
  ...
  document.addEventListener('keydown',function(e){
    if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==='k'){e.preventDefault();ov.classList.contains('open')?closePal():openPal();}
  });
